...
class Interactive {
    ...
    _onMapClick(e) {
        ...
        this._loadTile(coords.z, coords.x, coords.y)
            .then(() => this._objectForEvent(eventClone, 'click'));
    }
    ...
    _onMapMouseMove(e) {
        ...
        var coords = this._getTileCoordsFromMouseEvent(eventClone);
        this._loadTile(coords.z, coords.x, coords.y)
            .then(() => this._objectForEvent(eventClone, 'mousemove'));
    }
    ...
    _loadTile(z, x, y) {
        ...
        return fetch(this._buildTileUrl(z, x, y))
            ...
            .catch(data => this._eventEmitter.dispatchEvent('error', data));
    }
     ...
     */
    _objectForEvent(event, eventType) {
        ...
        var { x, y } = this._unscale(point, this._tileSize);
